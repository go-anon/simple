...
func (sn *SignalNotifier) Listen(ctx context.Context) {
	...
	go func() {
		defer signal.Stop(s)
		logs.Info("Listening for signals...")

		for {
			select {
			case <-ctx.Done():
				logs.Debug("Signal listener stopped due to ctx.Done()")
				return
			...
			case sig := <-s:
				...
				logs.Debug("Signal listener stopped due to signal: %v", sig)
				return
			}
		}
	}()
...
func defaultHandler(sig os.Signal) {
	logs.Error("Default handler, signal received: %v", sig)
